...
)
{
  ...
  // TODO: Address the compilation warnings generated by the following command.
  ...
  // `LinearVelocityCmd` sets linear velocity in the world frame, and is
  ...
  // `AngularVelocityCmd` sets angular velocity in the world frame, and is
  // unaffected by the rotation performed by `WorldPoseCmd`.
...
TEST_F(EmptyTestFixture, AccelerationTest) {
  ...
  for (std::size_t i = 0; i < kNCases; i++) {
